...
void filterRotRef(float rotd[], float dt)
{
  static float y_prev[3] = {0, 0, 0};
  for (int i = 0; i < 3; i++)
  {
    rotd[i] = (10 * rotd[i] + (y_prev[i] / dt)) / (10 + (1 / dt));
    y_prev[i] = rotd[i];
    // temp = rotd[i];
...
void handleESPMsg(char msgType, float data[], int len)
{
  ...
  else
  {
    switch (msgType)
    {
    case MsgType::CALIBRATE_ACC:
      Serial.println("calibrando");
      // if (!motorsActive) imu.calibrateAcc();
...
void loop()
{
  ...
  currentTime = micros();
  dt = (currentTime - timePrev) / 1000000;
  ...
  readRotRef(rotd);
  filterRotRef(rotd, dt);
  // for (int i = 0; i < 3; i++) rotd[i] = 0;

  float pid_out[3];
  for (int i = 0; i < 3; i++)
  {
    pid_out[i] = controllers[i].calc(rot[i], rotd[i], dt);
  ...
  if (motorsActive)
  {
    ...
    // motors[Prop::LF].writeMicroseconds(radio.inputs[RadioInput::THROTTLE] > 1700 ? 1200 : 1000);
    // motors[Prop::RF].writeMicroseconds(radio.inputs[RadioInput::YAW] > 1700 ? 1200 : 1000);
    // motors[Prop::LB].writeMicroseconds(radio.inputs[RadioInput::ROLL] > 1700 ? 1200 : 1000);
    // motors[Prop::RB].writeMicroseconds(radio.inputs[RadioInput::PITCH] > 1700 ? 1200 : 1000);
  ...
  // Serial2.write(MsgType::RESP_ROT);
  // Serial2.print(rot[0], 5);
  // Serial2.print(',');
  // Serial2.print(rot[1], 5);
  // Serial2.print(',');
  // Serial2.print(rot[2], 5);
  // Serial2.print('#');


  // Serial.print("Pitch:");
  // Serial.print(rot[0]*(RAD2DEG));
  // Serial.print("Ref:");
  // Serial.print(rotd[0]*(RAD2DEG));
  // Serial.print(" PID:");
  // Serial.print(pid_out[0]);
  ...
  // Serial.print(motorsActive);
  // Serial.print("t:");
  // Serial.print(radio.inputs[RadioInput::THROTTLE]);
  // Serial.print("a:");
  // Serial.print(radio.inputs[RadioInput::ARM]);
  // Serial.print(" Kp:");
